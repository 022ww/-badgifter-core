...
export default class SocketService {
  ...
  sendApiRequest(request) {
    return new Promise((resolve, reject) => {
      ...
      this.pair().then(() => {
        ...
        if (request.hasOwnProperty('payload') && !request.payload.hasOwnProperty('origin')) request.payload.origin = this.getOrigin();
  ...
  send(type = null, data = null) {
    if (type === null && data === null) this.socket.send('40/scatter');else this.socket.send('42/scatter,' + JSON.stringify([type, Object.assign(data, {
      device,
      uuid: this.uuid
    })]));
  }
